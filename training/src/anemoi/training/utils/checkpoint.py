...
def transfer_learning_loading(
    ...
    _di = checkpoint["hyper_parameters"]["data_indices"]
    if isinstance(_di, dict):
        model._ckpt_model_name_to_index = {k: v.name_to_index for k, v in _di.items()}
    else:
        model._ckpt_model_name_to_index = _di.name_to_index
    return model
