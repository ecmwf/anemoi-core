...
class MultiDataset(IterableDataset):
    ...
    def __iter__(self) -> dict[str, torch.Tensor]:
        ...
        if hasattr(self, 'ens_sample_comm_group_rank') and self.ens_sample_comm_group_rank != 0:
            # Build empty tensors with correct shapes for broadcasting
            sample = self.get_sample(shuffled_chunk_indices[0])
            empty_shapes = {name: tensor.shape for name, tensor in sample.items()}
            for _ in range(len(shuffled_chunk_indices)):
                yield {name: torch.empty(shape) for name, shape in empty_shapes.items()}
            return
