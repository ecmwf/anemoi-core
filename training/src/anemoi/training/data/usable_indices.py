...
def compute_valid_data_indices(
    ...
    for dataset_name, ds in data_readers.items():
        valid_date_indices[dataset_name] = get_usable_indices(
            ds.missing,
            len(ds.dates),
        ...
        if len(valid_date_indices[dataset_name]) == 0:
            msg = f"No valid date indices found for data reader '{dataset_name}': {ds}"
            raise ValueError(msg)

        LOGGER.info("Data reader '%s' has %d valid indices", dataset_name, len(valid_date_indices[dataset_name]))
