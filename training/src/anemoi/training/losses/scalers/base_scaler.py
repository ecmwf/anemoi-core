...
class BaseUpdatingScaler(BaseScaler):
    ...
    def on_training_start(self, model: ModelInterface, **kwargs) -> torch.Tensor | None:  # noqa: ARG002
        """Callback method called at the start of training."""
        LOGGER.debug("%s.on_training_start called.", self.__class__.__name__)

    def on_batch_start(self, model: ModelInterface, **kwargs) -> torch.Tensor | None:  # noqa: ARG002
