...
def _resolve_discriminated_union_model(
    ...
    *value* and matches it against each branch's ``Literal`` annotation.
    ...
    discriminator_value = next(
        (value.get(k) for k in keys if k in value and value.get(k) is not None), None
    )
...
def _deep_merge_defaults(default_value: Any, user_value: Any) -> Any:
    ...
    if isinstance(default_value, list) and isinstance(user_value, list):
        return [
            _deep_merge_defaults(default_value[i], v) if i < len(default_value) else v
            for i, v in enumerate(user_value)
        ]
    return user_value
...
def _try_resolve_element(annotation: Any, metadata: tuple[Any, ...], value: Any) -> Any | None:
    ...
    """
    return (
        _resolved_model_defaults_for_value(annotation, metadata, value)
        or _apply_container_defaults(annotation, metadata, value)
    )
