...
def _spawn_ensemble_ddp_gradients_match_unsharded_reference(path: Path) -> None:
    """Utility for tests.unit.distributed.test_ensemble_gradients.

    With importlib import mode, multiprocessing calls cannot be done in the test because
    multiprocessing needs to pickle the module path of the function called.

    Launch four CPU ranks and propagate any worker's assertion failure to pytest.
    """
    mp.spawn(_check_ensemble_gradients, args=(str(path / "rendezvous"),), nprocs=4, join=True)
