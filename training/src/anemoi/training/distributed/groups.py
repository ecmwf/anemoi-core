...
def create_reader_process_groups(reader_group_ranks: np.ndarray) -> list[list[Any]]:
    ...
    return [
        [torch.distributed.new_group(ranks, use_local_synchronization=True) for ranks in group_ranks]
...
def create_ensemble_process_groups(layout: EnsembleLayout) -> EnsembleProcessGroups:
    ...
    ens_comm_groups = [
        torch.distributed.new_group(ranks, use_local_synchronization=True) for ranks in layout.ens_comm_group_ranks
    ]
    ens_comm_subgroups = [
        torch.distributed.new_group(ranks, use_local_synchronization=True) for ranks in layout.ens_comm_subgroup_ranks
    ]
