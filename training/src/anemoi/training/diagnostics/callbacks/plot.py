...
class LongRolloutPlots(BasePlotCallback):
    ...
    def __init__(
        ...
        sample_idx : int
            Sample to plot
        parameters : dict
            Parameters to plot, one list per dataset
...
class GraphTrainableFeaturesPlot(BasePerEpochPlotCallback):
    ...
    def get_edge_trainable_modules(
        ...
        LOGGER.info("Getting edge trainable modules for dataset: %s", dataset_name)
        trainable_modules = {}
        ...
        if (
            ...
            LOGGER.info("Found an encoder graph_provider for dataset: %s ... ", dataset_name)
        ...
        if (
            ...
            LOGGER.info("Found a decoder graph_provider for dataset: %s ... ", dataset_name)
...
class PlotLoss(BasePerBatchPlotCallback):
    ...
    def __init__(
        ...
        self.dataset_names = dataset_names if dataset_names is not None else ["era5"]
    ...
    def _plot(
        ...
        LOGGER.info("Plotting loss per parameter at epoch %d, batch %d ...", epoch, batch_idx)
        LOGGER.info("Dataset names: %s", dataset_names)
        LOGGER.info("Data indices pl_module: %s", list(pl_module.data_indices.keys()))
...
class PlotSample(BasePlotAdditionalMetrics):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.sample_idx = sample_idx
        self.parameters = parameters
        LOGGER.info("!!!***** PARAMETERS TO PLOT: %s", self.parameters)
    ...
    def _plot(
        ...
        for dataset_name in dataset_names:
            ...
            assert dataset_name in self.parameters, (
                "Dataset name %s not found in parameters to plot dictionary keys: %s",
                dataset_name,
        ...
        for dataset_name in dataset_names:
            ...
            for rollout_step in range(rollout):
                ...
                self._output_figure(
                    ...
                    exp_log_tag=f"pred_val_histo_{dataset_name}_rstep_