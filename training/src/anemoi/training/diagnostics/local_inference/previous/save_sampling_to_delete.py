...
# Main function
def main(name_exp, N_samples, N_members):
    ...
    # )
    inference_loader = trainer.datamodule.predict_dataloader()
    trainer = pl.Trainer(accelerator="gpu", strategy="ddp", devices=4)
    samples = trainer.predict(
        downscaler,
        inference_loader,
    )
...
if __name__ == "__main__":
    ...
    main(args.name_exp, args.N_samples, args.N_members)
    ...
    # srun --partition=gpu --nodes=1 --gpus-per-node=4 -c 8 --mem=128G -t 00:30:0 --ntasks-per-node=4  python /home/ecm5702/dev/anemoi-training/src/anemoi/training/diagnostics/nb_plots/save_sampling_ddp.py --name_exp 30040c70a9ca4c8092ca1688a2f9c190 --N_samples 1 --N_members 3
