...
def get_mlflow_logger(config: BaseSchema) -> None:
    ...
    save_dir = config.system.storage.logs.mlflow
    ...
    if (resumed or forked) and (offline):  # when resuming or forking offline -
        # tracking_uri = ${system.storage.logs.mlflow}
        tracking_uri = str(save_dir)
    # create directory if it does not exist
    Path(config.system.storage.logs.mlflow).mkdir(parents=True, exist_ok=True)
    ...
    if config.diagnostics.log.mlflow.terminal:
        logger.log_terminal_output(artifact_save_dir=config.system.storage.plots)
...
def get_tensorboard_logger(config: DictConfig) -> pl.loggers.TensorBoardLogger | None:
    ...
    return TensorBoardLogger(
        save_dir=config.system.storage.logs.tensorboard,
        log_graph=False,
    )
...
def get_wandb_logger(config: DictConfig, model: pl.LightningModule) -> pl.loggers.WandbLogger | None:
    ...
    logger = WandbLogger(
        ...
        save_dir=config.system.storage.logs.wandb,
