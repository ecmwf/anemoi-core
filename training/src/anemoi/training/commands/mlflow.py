...
class MlFlow(Command):
    ...
    def run(args: argparse.Namespace) -> None:  # noqa: C901
        ...
        if args.subcommand == "sync":
            ...
            source = args.source
            if source.startswith("sqlite://") or source.startswith("http"):
                ...
                has_legacy_store = any(d.is_dir() and d.name.isdigit() for d in source_path.iterdir()) if source_path.is_dir() else False
                if sqlite_db.exists():
                    ...
                    msg = f"Source '{source}' does not look like a valid MLflow store (no mlflow.db and no numeric experiment directories found)."
                    raise ValueError(msg)
