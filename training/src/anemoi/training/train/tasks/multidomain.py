...
class GraphMultiDomainForecaster(BaseGraphModule):
    ...
    def rollout_step(
        ...
        import numpy as np
        dataset_name = np.random.choice(list(batch.keys()))
        x = {}
        ...
        for key, value in y_pred.items():
            LOGGER.info(f"{key} {value.mean()} {torch.isnan(value).any()}")
        y = {}
        y[dataset_name] = batch[dataset_name][
            :,
            self.multi_step,
            ...,
        ...
        total_loss, metrics_next = checkpoint(
