...
def test_offline_legacy_store_warning(tmp_path_factory: pytest.TempPathFactory, caplog: pytest.LogCaptureFixture) -> None:
    import logging

    legacy_dir = tmp_path_factory.mktemp("legacy_mlruns")
    (legacy_dir / "0").mkdir()  # numeric subdir mimics old filesystem store layout
    with caplog.at_level(logging.WARNING, logger="anemoi.training.diagnostics.mlflow.logger"):
        try:
            AnemoiMLflowLogger(
            ...
            pass
    assert any("legacy" in msg.lower() for msg in caplog.messages), (
