...
def _build_edge_data(src_size: int, dst_size: int, device: torch.device) -> tuple[torch.Tensor, torch.Tensor]:
...
def _build_tiny_graph(dataset_name: str, device: torch.device) -> dict[str, HeteroData]:
    ...
    edge_index_dh, edge_attr_dh = _build_edge_data(num_data_nodes, num_hidden_nodes, device)
    edge_index_hh, edge_attr_hh = _build_edge_data(num_hidden_nodes, num_hidden_nodes, device)
    edge_index_hd, edge_attr_hd = _build_edge_data(num_hidden_nodes, num_data_nodes, device)
...
def _compute_sharded_grads(
    ...
    out_full = gather_tensor(out_local, dim=3, shapes=out_shapes, mgroup=model_comm_group).detach()
...
def _run_diffusion_gradient_scaling_parity(
    ...
    x_full = (
        torch.arange(num_nodes * num_vars, dtype=torch.float32, device=device).reshape(1, 1, 1, num_nodes, num_vars)
        / 19.0
    )
    ...
    torch.testing.assert_close(hooked_out, ref_out, atol=8e-4, rtol=8e-4)
    torch.testing.assert_close(unhooked_out, ref_out, atol=8e-4, rtol=8e-4)
    ...
    for name, ref_grad in ref_grads.items():
        ...
        torch.testing.assert_close(hooked_grads_avg[name], ref_grad, atol=1e-3, rtol=1e-3)

        if (not _is_skip_scaled(name)) and (
            not torch.allclose(unhooked_grads_avg[name], ref_grad, atol=1e-3, rtol=1e-3)
        ):
...
def _compute_ensemble_mode(
    *,
    ...
    model_layout = build_model_layout(world_size=world_size, global_rank=rank, model_comm_group_size=1)
    ...
    loss = ((y_pred_ens - target_two_members) ** 2).sum() / float(target_two_members.numel())
    loss.backward()
...
def _run_ensemble_partitioning_parity(
    ...
    num_nodes = graph_data[dataset_name]["data"].x.shape[0]
    x_member0 = (
        torch.arange(num_nodes * num_vars, dtype=torch.float32, device=device).reshape(1, 1, num_nodes, num_vars) / 17.0
    )
    x_member1 = torch.cos(x_member0 * 0.8 + 0.3)
    x_two_members = torch.stack((x_member0, x_member1), dim=2)  # (batch, time, ens=2, grid, vars)
    ...
    torch.testing.assert_close(out_local_ens, out_dist_ens, atol=1e-4, rtol=1e-4)
    for name, grad_local in grads_local_ens.items():
        torch.testing.assert_close(grads_dist_ens[name], grad_local, atol=1e-4, rtol=1e-4)
...
def _run_ensemble_world_step_scaling_parity(
    ...
    num_nodes = graph_data[dataset_name]["data"].x.shape[0]
    x_member0 = (
        torch.arange(num_nodes * num_vars, dtype=torch.float32, device=device).reshape(1, 1, num_nodes, num_vars) / 13.0
    )
    ...
    for name, delta_ref in delta_local.items():
        ...
        if not torch.allclose(delta_dist[name], delta_ref, atol=1e-5, rtol=1e-5):
        ...
        torch.testing.assert_close(delta_dist[name] * 4.0, delta_ref, atol=1e-4, rtol=1e-4)
...
def _run_rank(rank: int, world_size: int, backend: str, init_file: str, suite: str) -> None:
    if backend == "nccl":
    ...
    try:
        if suite == "diffusion":
            _run_diffusion_gradient_scaling_parity(rank, world_size, device, dist.group.WORLD)
        elif suite == "ensemble":
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Distributed strategy gradient parity runner")
    parser.add_argument("--backend", choices=["gloo", "nccl"], required=True)
    parser.add_argument("--suite", choices=["diffusion", "ensemble", "ensemble_world_step"], default="diffusion")
    ...
    try:
        mp.spawn(
            _run_rank,
            args=(args.world_size, args.backend, str(init_file), args.suite),
