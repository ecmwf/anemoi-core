...
Proper configuration management is essential to ensure that integration
tests remain reliable and maintainable. Our approach includes:
...
dataset-specific and test-specific modifications on top of the template.
Use a config modification yaml, or hydra overrides for parametrization
of a small number of config values.

3. Reducing Compute Load: Where possible, reduce the number of batches,
epochs, and batch sizes by comparing configurations against test
defaults.
...
files for inconsistencies and verify if test-time modifications have
...
model. It applies test-specific modifications to reduce batch_size etc.
as detailed in `training/tests/integration/test_training_cycle.yaml`. It
furthermore applies dataset-specific modifications as detailed in
