...
Use the validate command to resolve configs and apply lightweight adapter checks before
...
This command resolves interpolations, expands paths, and can mask missing environment
variables via ``--mask_env_vars``. It does not perform schema-based validation; deeper
errors surface when the pipeline is constructed or during training.
