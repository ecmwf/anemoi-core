...
# validation sample/lead/variable for the finer_graph_v1 setup.
# Usage:
#   run_rrfs_verify_loss_table_1h_refc_value_no_refc_input_finer_graph_v1.sh <checkpoint_path> <start> <end> <frequency>
set -euo pipefail

if [[ $# -ne 4 ]]; then
  echo "Usage: run_rrfs_verify_loss_table_1h_refc_value_no_refc_input_finer_graph_v1.sh <checkpoint_path> <start> <end> <frequency>"
  exit 1
fi
...
END="$3"
FREQ="$4"
CONFIG_NAME="anemoi-training-rrfs-lam-neural-lam-verify-202405-1h-refc-value-loss-no-refc-input-finer-graph-v1-loss-table"
RESOLVED_CONFIG="/scratch3/NCEPDEV/fv3-cam/Ting.Lei/tmp-verify-resolved-1h-refc-value-no-refc-input-finer-graph-v1-loss-table.yaml"

echo "DEBUG_CONFIG_NAME: $CONFIG_NAME"
echo "DEBUG_LOSS_TABLE_DIR: /scratch3/NCEPDEV/fv3-cam/Ting.Lei/tlei-anemoi-training/verify/loss_tables/1h_refc_value_no_refc_input_finer_graph_v1"
