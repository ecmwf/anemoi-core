#!/usr/bin/env bash
# Run 1h-to-3h verification for checkpoints trained with refc as diagnostic/output-only
# and refc value/range-weighted loss.
# Usage:
#   run_rrfs_verify_export_1to3h_refc_value_no_refc_input.sh <checkpoint_path> <start> <end> <frequency>
set -euo pipefail

if [[ $# -ne 4 ]]; then
  echo "Usage: run_rrfs_verify_export_1to3h_refc_value_no_refc_input.sh <checkpoint_path> <start> <end> <frequency>"
  exit 1
fi
...
END="$3"
FREQ="$4"
CONFIG_NAME="anemoi-training-rrfs-lam-neural-lam-verify-202405-1to3h-refc-value-loss-no-refc-input"
...
anemoi-training train \
  ...
  --cfg job > /scratch3/NCEPDEV/fv3-cam/Ting.Lei/tmp-verify-resolved-1to3h-refc-value-no-refc-input.yaml

echo "DEBUG_CONFIG_SAVED: /scratch3/NCEPDEV/fv3-cam/Ting.Lei/tmp-verify-resolved-1to3h-refc-value-no-refc-input.yaml"
