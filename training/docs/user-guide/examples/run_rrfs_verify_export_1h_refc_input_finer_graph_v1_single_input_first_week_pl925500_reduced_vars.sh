...
# prognostic/input variable, finer_graph_v1, single-input setup, and reduced
# variables (no hydrometeors, no 850/200 pressure-level fields).
# Usage:
#   run_rrfs_verify_export_1h_refc_input_finer_graph_v1_single_input_first_week_pl925500_reduced_vars.sh <checkpoint_path> <start> <end> <frequency>
set -euo pipefail

if [[ $# -ne 4 ]]; then
  echo "Usage: run_rrfs_verify_export_1h_refc_input_finer_graph_v1_single_input_first_week_pl925500_reduced_vars.sh <checkpoint_path> <start> <end> <frequency>"
  exit 1
fi
...
END="$3"
FREQ="$4"
CONFIG_NAME="anemoi-training-rrfs-lam-neural-lam-verify-202405-1h-refc-input-finer-graph-v1-single-input-first-week-pl925500-reduced-vars"
RESOLVED_CONFIG="/scratch3/NCEPDEV/fv3-cam/Ting.Lei/tmp-verify-resolved-1h-refc-input-fgv1-single-input-first-week-pl925500-reduced-vars.yaml"

echo "DEBUG_CONFIG_NAME: $CONFIG_NAME"
echo "DEBUG_VERIFY_ROOT: /scratch3/NCEPDEV/fv3-cam/Ting.Lei/tlei-anemoi-training/verify/finer_graph_v1_single_input_first_week_refc_input_pl925500_reduced_vars/"
