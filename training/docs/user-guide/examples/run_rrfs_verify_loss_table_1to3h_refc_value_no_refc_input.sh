#!/usr/bin/env bash
# Run 1h-to-3h verification and export configured loss tables for every
# validation sample/lead/variable.
# Usage:
#   run_rrfs_verify_loss_table_1to3h_refc_value_no_refc_input.sh <checkpoint_path> <start> <end> <frequency>
set -euo pipefail

if [[ $# -ne 4 ]]; then
  echo "Usage: run_rrfs_verify_loss_table_1to3h_refc_value_no_refc_input.sh <checkpoint_path> <start> <end> <frequency>"
  exit 1
fi
...
END="$3"
FREQ="$4"
CONFIG_NAME="anemoi-training-rrfs-lam-neural-lam-verify-202405-1to3h-refc-value-loss-no-refc-input-loss-table"
RESOLVED_CONFIG="/scratch3/NCEPDEV/fv3-cam/Ting.Lei/tmp-verify-resolved-1to3h-refc-value-no-refc-input-loss-table.yaml"

echo "DEBUG_CONFIG_NAME: $CONFIG_NAME"
echo "DEBUG_LOSS_TABLE_DIR: /scratch3/NCEPDEV/fv3-cam/Ting.Lei/tlei-anemoi-training/verify/loss_tables/1to3h_refc_value_no_refc_input"
