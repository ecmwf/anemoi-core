#!/usr/bin/env bash
# Run direct +1h verification for GNN checkpoints trained with refc as
# diagnostic/output-only, refc value/range-weighted loss, and finer_graph_v1.
# Usage:
#   run_rrfs_verify_export_1h_refc_value_no_refc_input_gnn_finer_graph_v1.sh <checkpoint_path> <start> <end> <frequency>
set -euo pipefail

if [[ $# -ne 4 ]]; then
  echo "Usage: run_rrfs_verify_export_1h_refc_value_no_refc_input_gnn_finer_graph_v1.sh <checkpoint_path> <start> <end> <frequency>"
  exit 1
fi
...
END="$3"
FREQ="$4"
CONFIG_NAME="anemoi-training-rrfs-lam-neural-lam-verify-202405-1h-refc-value-loss-no-refc-input-gnn-finer-graph-v1"
RESOLVED_CONFIG="/scratch3/NCEPDEV/fv3-cam/Ting.Lei/tmp-verify-resolved-1h-refc-value-no-refc-input-gnn-finer-graph-v1.yaml"

echo "DEBUG_CONFIG_NAME: $CONFIG_NAME"
echo "DEBUG_VERIFY_ROOT: /scratch3/NCEPDEV/fv3-cam/Ting.Lei/tlei-anemoi-training/verify/gnn_finer_graph_v1_1h_refc_value_no_refc_input/"
