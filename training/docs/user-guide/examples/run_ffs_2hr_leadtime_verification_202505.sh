...
# for 2-hour lead prediction vs truth.
#
# Usage:
#   bash training/docs/user-guide/examples/run_ffs_2hr_leadtime_verification_202505.sh \
...
if [[ $# -lt 3 || $# -gt 6 ]]; then
  echo "Usage: run_ffs_2hr_leadtime_verification_202505.sh <checkpoint_path> <start> <end> [frequency] [graph_path] [dataset_path]"
  exit 1
fi
...
RUN_TAG="ffs_2hr_verification_202505_${START_TAG}_${END_TAG}"
...
echo "Running verification export (2h lead)"
...
CMD=(
  ...
  training.rollout.epoch_increment=0
...
echo "Generating per-valid-time frames (2h lead) into: $FRAMES_ROOT"
...
lead_idx = 1  # 2-hour lead for n_step_output=1 and 1h frequency
...
for path in files:
    ...
    if ds.sizes.get("target_time", 0) <= lead_idx or ds.sizes.get("pred_time", 0) <= lead_idx:
        continue

    valid_np = np.datetime64(ds.coords["target_time"].values[lead_idx])
    ...
    for var_name, alias in var_map:
        ...
        targ = ds["target"].isel(target_time=lead_idx).values[:, i]
        pred = ds["prediction"].isel(pred_time=lead_idx).values[:, i]
        err = targ - pred

        out_dir = frames_root / alias
        out_dir.mkdir(parents=True, exist_ok=True)
        out_file = out_dir / f"{alias}_lead02_valid_{valid_s}_frame_{frame_idx:05d}.png"

        fig, axs = plt.subplots(1, 3, figsize=(15, 5), layout="tight")
        titles = [
            f"truth {alias}\\nvalid {np.datetime_as_string(valid_np, unit='s')}",
            f"prediction {alias} (lead 2h)\\nvalid {np.datetime_as_string(valid_np, unit='s')}",
...
print("  ffmpeg -framerate 6 -pattern_type glob -i 'video_frames/hgt500/*_lead02_*.png' -c:v libx264 -pix_fmt yuv420p hgt500_lead02.mp4")
PY
