...
def add_1_hop_edges(
    ...
    LOGGER.debug("node_resolutions %s", node_resolutions)
    for subdivisions in node_resolutions:
        ...
        if subdivisions in edge_resolutions:
            LOGGER.debug("Subdivisions %d is in edge resolutions", subdivisions)
            resolution_mask.append(np.full(sphere.edges.shape[0], True, dtype=bool))
        else:
            LOGGER.debug("Subdivisions %d is not in edge resolutions", subdivisions)
            resolution_mask.append(np.full(sphere.edges.shape[0], False, dtype=bool))
    ...
    resolution_mask = np.concatenate(resolution_mask, axis=0)
    ...
    edges_mask = np.logical_and(valid_edges_mask, resolution_mask)
    # Select only those edges requested (domain and resolutions)
    multiscale_edges = updated_edges[:, edges_mask]
