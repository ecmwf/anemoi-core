...
def resolve_target(target: Any) -> Any:
    ...
    if not isinstance(target, str):
        msg = f"Target must be a dotted import path or callable, got {type(target).__name__}."
        raise TypeError(msg)

    module_name, _, attr = target.rpartition(".")
    if not module_name:
        msg = f"Target must be a dotted import path, got '{target}'."
        raise ValueError(msg)
    module = import_module(module_name)
    try:
        return getattr(module, attr)
    except AttributeError as exc:
        msg = f"Target '{target}' could not be resolved."
...
def build_component(component_config: Any, *, partial_override: bool | None = None, **kwargs: Any) -> Any:
    """Instantiate a component from a config mapping with a target path."""
    cfg = _normalize_config(component_config)
    ...
    if target is None:
        msg = "Component config must define '_target_' (or 'target_')."
        raise ValueError(msg)

    partial_flag = bool(cfg.pop("_partial_", False))
    recursive_flag = cfg.pop("_recursive_", True)
...
__all__ = ["build_component", "resolve_target"]
