...
class CkptMigrator(Migrator[CkptMigration, CkptType]):
    ...
    def inspect(self, path: str | Path) -> tuple[list[Migration], list[Migration], list[str]]:
        ...
        return (
            list(self.registered_migrations(ckpt)),
            list(self.missing_migrations(ckpt)),
            self.extra_migrations(ckpt),
        )
