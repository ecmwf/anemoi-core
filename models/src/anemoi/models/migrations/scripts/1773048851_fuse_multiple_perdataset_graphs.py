    ...
    for old_key in list(ckpt["state_dict"].keys()):
        ...
        for dataset_name in dataset_names:
            for key in ["data", "hidden"]:
                ...
                old_key_trainabletensors = f"model.model.node_attributes.{dataset_name}.trainable_tensors.{key}.trainable"

                # New checkpoints. Only one `hidden` key now.
                new_key_data_latlons = f"model.model.node_attributes.latlons_{dataset_name}"
                new_key_data_trainabletensors = f"model.model.node_attributes.trainable_tensors.{dataset_name}.trainable"
