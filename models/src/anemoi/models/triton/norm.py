...
def _rms_norm_fwd_standalone(x_ptr, out_ptr, w_ptr, H: tl.constexpr, C: tl.constexpr):
    ...
    tl.store(out_ptr + off, x.reshape(H * C))
...
class RMSNormFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, weight):
        ...
        _rms_norm_fwd_standalone[(1,)](x, out, weight, H, C)
    ...
    def backward(ctx, dout):
        ...
        _rms_norm_bwd_standalone[(1,)](x, dout, weight, dx, dw, H, C)
...
class RMSNorm(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.dim() != 2 or x.shape[1] != self.dim:
            raise ValueError(f"Expected input of shape (H, {self.dim}), got {tuple(x.shape)}")
