    ...
    if any(isinstance(entry, list) for entry in entries):
        ...
        for entry in entries:
            if len(entry) != len(reference):
                msg = f"Sparse transport data lists must have the same length, got {len(reference)} and {len(entry)}."
                raise ValueError(msg)

        outputs = []
        for sample_index, samples in enumerate(zip(*entries, strict=True)):
            sample_scalar = _sample_scalar_for_data(samples[0], scalar, sample_index)
