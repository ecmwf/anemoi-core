...
def _run_core_primitives(
    ...
    torch.testing.assert_close(y_gathered, y_full)
    y_gathered.sum().backward()
    torch.testing.assert_close(y_local.grad, torch.ones_like(y_local))
    ...
    torch.testing.assert_close(z_reduced, torch.full_like(z, expected_sum))
    ...
    torch.testing.assert_close(redshard_out, expected_reduced_local)
    redshard_out.sum().backward()
    torch.testing.assert_close(redshard_input.grad, torch.ones_like(redshard_input))
...
def _run_transformer_primitives(
    ...
    torch.testing.assert_close(x_heads_local, x_heads_expected)

    x_seq_roundtrip = shard_sequence(x_heads_local, shapes=seq_shapes, num_heads=num_heads, mgroup=model_comm_group)
    torch.testing.assert_close(x_seq_roundtrip, x_seq_local.detach())
    x_seq_roundtrip.sum().backward()
    torch.testing.assert_close(x_seq_local.grad, torch.ones_like(x_seq_local))
...
def _run_channel_primitives(
    ...
    torch.testing.assert_close(x_channel_local, x_channel_expected)
