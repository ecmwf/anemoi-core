...
def test_triton_attention_varlen_fwd_masked(H, seqlens, HEAD_DIM, dtype, causal, window):
    """Tests varlen flash attention forward pass with causal and/or sliding window masking."""
    if not is_triton_available():
        pytest.skip("Triton not available")
    ...
    # Determine window size: pick a value relative to the shortest sequence
    min_seqlen = min(seqlens)
    if window:
        torch.manual_seed(0)
        window_size = max(1, min_seqlen // 4)  # use quarter of shortest seq
    else:
        window_size = -1
    ...
    ref_out = attention_varlen_ref(q, k, v, cu_seqlens_q, cu_seqlens_k, sm_scale, causal=causal, window_size=window_size)

    # Compute triton output
    tri_out = TritonAttentionVarlen.apply(q, k, v, cu_seqlens_q, cu_seqlens_k, causal, window_size, sm_scale)
    ...
    try:
        ...
        with torch.no_grad():
            ...
            t, h, d = [int(x) for x in max_idx]
            print(
                f"[varlen-attn-masked debug] global max abs error: {float(max_err.cpu())}"
                f" at (token, head, dim)=({t}, {h}, {d})"
            )
            print(f"[varlen-attn-masked debug] tri_out[t, h, :8] = {tri_out[t, h, :8].cpu()}")
            print(f"[varlen-attn-masked debug] ref_out[t, h, :8] = {ref_out[t, h, :8].cpu()}")
            for b, (s_start, s_end) in enumerate(zip(cu_seqlens[:-1], cu_seqlens[1:])):
                seq_diff = diff[s_start:s_end].max()
                print(f"[varlen-attn-masked debug] seq {b} (len={s_end - s_start}) max error: {float(seq_diff.cpu())}")
        raise

    print(f"[varlen-attn fwd masked] PASSED: H={H}, seqlens={seqlens}, HEAD_DIM={HEAD_DIM}, dtype={dtype}, causal={causal}, window_size={window_size}")
...
@pytest.mark.parametrize("window", [True, False])
def test_triton_attention_varlen_bwd_masked(H, seqlens, HEAD_DIM, dtype, causal, window):
    """Tests varlen flash attention backward pass with causal and/or sliding window masking."""
    if not is_triton_available():
        pytest.skip("Triton not available")
    ...
    min_seqlen = min(seqlens)
    if window:
        window_size = max(1, min_seqlen // 4)
    else:
        window_size = -1
    ...
    ref_out = attention_varlen_ref(q_ref, k_ref, v_ref, cu_seqlens_q, cu_seqlens_k, sm_scale, causal=causal, window_size=window_size)
    ref_out.backward(dout)
    ...
    tri_out = TritonAttentionVarlen.apply(q_tri, k_tri, v_tri, cu_seqlens_q, cu_seqlens_k, causal, window_size, sm_scale)
    tri_out.backward(dout)
    ...
    try:
        ...
        with torch.no_grad():
            diff = (tri_out - ref_out).abs()
            max_err = diff.max()
            print(f"[varlen-attn bwd masked debug] forward max error: {float(max_err.cpu())}")
        raise
    ...
    try:
        ...
        with torch.no_grad():
            ...
            max_idx = (diff == max_err).nonzero(as_tuple=False)[0]
            t, h, d = [int(x) for x in max_idx]
            print(f"[varlen-attn bwd masked debug] dQ max error: {float(max_err.cpu())} at ({t}, {h}, {d})")
            for b, (s_start, s_end) in enumerate(zip(cu_seqlens[:-1], cu_seqlens[1:])):
                seq_diff = diff[s_start:s_end].max()
                print(f"[varlen-attn bwd masked debug] dQ seq {b} (len={s_end - s_start}) max error: {float(seq_diff.cpu())}")
        raise
    ...
    try:
        ...
        with torch.no_grad():
            ...
            max_idx = (diff == max_err).nonzero(as_tuple=False)[0]
            t, h, d = [int(x) for x in max_idx]
            print(f"[varlen-attn bwd masked debug] dK max error: {float(max_err.cpu())} at ({t}, {h}, {d})")
            for b, (s_start, s_end) in enumerate(zip(cu_seqlens[:-1], cu_seqlens[1:])):
                seq_diff = diff[s_start:s_end].max()
                print(f"[varlen-attn bwd masked debug] dK seq {b} (len={s_end - s_start}) max error: {float(seq_diff.cpu())}")
        raise
    ...
    try:
        ...
        with torch.no_grad():
            ...
            max_idx = (diff == max_err).nonzero(as_tuple=False)[0]
            t, h, d = [int(x) for x in max_idx]
            print(f"[varlen-attn bwd masked debug] dV max error: {float(max_err.cpu())} at ({t}, {h}, {d})")
            for b, (s_start, s_end) in enumerate(zip(cu_seqlens[:-1], cu_seqlens[1:])):
                seq_diff = diff[s_start:s_end].max()
                print(f"[varlen-attn bwd masked debug] dV seq {b} (len={s_end - s_start}) max error: {float(seq_diff.cpu())}")
        raise

    print(f"[varlen-attn bwd masked] PASSED: H={H}, seqlens={seqlens}, HEAD_DIM={HEAD_DIM}, dtype={dtype}, causal={causal}, window_size={window_size}")
